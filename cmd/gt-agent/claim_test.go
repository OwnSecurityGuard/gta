...
func TestClaimAccessCodeParsesConfig(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("code") != "GT-ABC1-DEF2" {
	...
	cfg, err := claimAccessCode(context.Background(), hostPort, "GT-ABC1-DEF2")
	if err != nil {
		t.Fatal(err)
	}
	...
	if cfg.Token != "gt_team" || cfg.SessionID != "s-1" {
...
func TestClaimAccessCodeHttpError(t *testing.T) {
	...
	hostPort := hostPortOf(t, srv.URL)
	if _, err := claimAccessCode(context.Background(), hostPort, "GT-NOPE-0000"); err == nil {
