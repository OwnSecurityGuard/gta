...
// fakePluginTmpl 是被测插件源码模板：把收到的 GT_* 环境变量写入编译时
...
func main() {
	...
	if f != nil {
		f.WriteString("registry=" + os.Getenv("GT_REGISTRY_ADDR") + "\n")
		f.WriteString("tunnel=" + os.Getenv("GT_TUNNEL") + "\n")
		f.WriteString("token=" + os.Getenv("GT_AUTH_TOKEN") + "\n")
		f.Close()
	}
	if MODE == "exit" {
		os.Exit(3)
...
func TestSpawnEnvAnonymous(t *testing.T) {
	...
	if !containsKV(env, "GT_REGISTRY_ADDR=host:9091") || !containsKV(env, "GT_TUNNEL=1") {
		t.Fatalf("missing required env: %v", env)
	}
	if containsKV(env, "GT_AUTH_TOKEN=") {
		t.Fatalf("anonymous mode must not set GT_AUTH_TOKEN: %v", env)
	}
}
...
func TestSpawnEnvPropagationFull(t *testing.T) {
	...
	sup := &pluginSupervisor{dir: dir, registryAddr: "10.1.2.3:9091", token: "gt_test"}
	stop := startSupervisor(t, sup, 1)
	defer stop()
