...
type pipelineService struct {
	...
	// GT_DB_*/默认 sqlite）解析而来，传给每个会话 store 的工厂方法。
	dbDriver string
	dbDSN    string

	// registryAddr 是插件应连接的注册中心地址（即 -registry-addr 的值，如 :9091）。
	// 通过 GetRegistryAddr 暴露给 gt-mcp，供其 / 插件启动时获知 GT_REGISTRY_ADDR。
	registryAddr string

	// agentHub 非 nil 时，每个抓包会话额外打开 agent source，
	// 接收 gt-agent 经 AgentIngest server 推送的本机原始帧（-agent-ingest-addr 启用时）。
	agentHub *agent.Hub
	...
	// 每个租约 = 常驻 gt-singbox-agent 进程 + 固定的手机 CONNECT 端口 +
	...
	agentBin     string              // gt-singbox-agent 二进制路径（空 = <workdir>/bin 下查找）
...
// gt-mcp 借此向调用方/插件暴露 GT_REGISTRY_ADDR，避免插件「不知道该连哪里」。
