// Command gt-singbox-agent 是移动端流量入口（sing-box 侧）到 GameTrace 的桥接程序。
//
// 它是一个**常驻的手机出口**：监听地址在进程生命周期内固定不变，抓包与否
// 由 gt-pipeline 通过本地控制接口（--control）随时切换。手机配好一次代理
// （二维码）之后，反复开始/停止抓包都不需要重新扫码、不需要重连 VPN。
//
// 常驻模式（推荐，由 gt-pipeline 的代理租约拉起）：
//
//	gt-singbox-agent --listen 0.0.0.0:12100 --control 127.0.0.1:19500
//
...
//
//	gt-singbox-agent --server 127.0.0.1:9090 --listen 127.0.0.1:12000
//
// 纯代理模式下 --target 省略：动态解析手机 CONNECT 请求中的目标地址，
// 适合"常驻等待代理连接"，GameTrace 未运行或未抓包时不影响监听。
//
// 联调模式（无 sing-box，本地模拟游戏流量）：
//
//	gt-singbox-agent --sim --target 127.0.0.1:19000
//
...
func main() {
	...
	flag.StringVar(&server, "server", "", "GameTrace mobile capture source gRPC address (empty = start idle, wait for capture/start via --control)")
	...
	<-ctx.Done()
	logger.Info("gt-singbox-agent shutting down")
}
