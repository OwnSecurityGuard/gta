...
// 架构：
//
//	手机 ── HTTP CONNECT（端口恒定）──▶ gta-singbox-agent（常驻进程）
//	                                        │ 本地控制接口（195xx）
//	                                        ▼
...
// spawnSingboxAgentLease 以常驻 idle 模式拉起租约专属 gta-singbox-agent。
...
// 避免测试进程与开发机常驻服务（如正在跑的 gta-singbox-agent）争夺 GTA 专属端口。
...
func (s *pipelineService) CreateProxyLease(ctx context.Context, req capturecontrol.CreateProxyLeaseRequest) (capturecontrol.ProxyLease, error) {
	...
	if !ap.running() {
		ap.stop()
		releasePorts()
		return capturecontrol.ProxyLease{}, fmt.Errorf("gta-singbox-agent exited immediately (listen %s, control %s)", listenAddr, ctrlAddr)
	}
